...
from functions_all.sreality_api_dictionaries import *
from functions_all.sreality_description_asyncio import *
    ...
    with tqdm(total=total, desc="decoding responses with description of offers: ") as pbar:
        for r_dict in responses_list:
            try:
                ...
                if "Vlak" in [i["name"] for i in r_dict["poi"]]:
                    info_relevant["Vlak"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Vlak"][0]
                else:
                    info_relevant["Vlak"] = 0
                    
                if "Večerka" in [i["name"] for i in r_dict["poi"]]:   
                    info_relevant["Večerka"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Večerka"][0]
                else:
                    info_relevant["Večerka"] = 0       
                
                if "Obchod" in [i["name"] for i in r_dict["poi"]]:   
                    info_relevant["Obchod"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Obchod"][0]
                else:
                    info_relevant["Obchod"] = 0 
                
                if "Bus MHD" in [i["name"] for i in r_dict["poi"]]:   
                    info_relevant["Bus MHD"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Bus MHD"][0]
                else:
                    info_relevant["Bus MHD"] = 0 

                if "Škola" in [i["name"] for i in r_dict["poi"]]:   
                    info_relevant["Škola"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Škola"][0]
                else:
                    info_relevant["Škola"] = 0 
                
                if "Školka" in [i["name"] for i in r_dict["poi"]]:   
                    info_relevant["Školka"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Školka"][0]
                else:
                    info_relevant["Školka"] = 0 
                        
                if "Lékař" in [i["name"] for i in r_dict["poi"]]:   
                    info_relevant["Lékař"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Lékař"][0]
                else:
                    info_relevant["Lékař"] = 0 
                    
                if "Hospoda" in [i["name"] for i in r_dict["poi"]]:   
                    info_relevant["Hospoda"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Hospoda"][0]
                else:
                    info_relevant["Hospoda"] = 0 
                
                if "Restaurace" in [i["name"] for i in r_dict["poi"]]:   
                    info_relevant["Restaurace"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Restaurace"][0]
                else:
                    info_relevant["Restaurace"] = 0
                    
                if "Metro" in [i["name"] for i in r_dict["poi"]]:   
                    info_relevant["Metro"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Metro"][0]
                else:
                    info_relevant["Metro"] = 0
                    
                if "Tram" in [i["name"] for i in r_dict["poi"]]:   
                    info_relevant["Tram"] = [i["distance"] for i in r_dict["poi"] if i["name"] == "Tram"][0]
                else:
                    info_relevant["Tram"] = 0
