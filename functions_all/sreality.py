...
from functions_all.sreality_description_download_decoding import *
...
# def get_house_area_from_name(name: str):
#     """(\d*) matches any decimal digits (repeated after each other"""
#     name_ = name.split()
#     return int(''.join(re.findall('(\d*)', ''.join(name_[-5:-4]))))

def get_land_area_from_name(name: str):
    """(\d*) matches any decimal digits (repeated after each other"""
    name_ = name.split()
    return int(''.join(re.findall('(\d*)', ''.join(name_[-2:]))))
...
def decode_collector(collector: list, category_main: int) -> pd.DataFrame:
    ...
    for page, r in tqdm(collector.items(),desc="decoding pages with offers: "):
        for estate in r['_embedded']['estates']: 
            ...
            # estate_relevant['house_area'] = get_house_area_from_name(estate['name'])
            estate_relevant['land_area'] = 0 #get_land_area_from_name(estate['name'])
            ...
            estate_relevant['category_main'] = estate["seo"]["category_main_cb"] # ? house
            estate_relevant['category_sub'] = estate["seo"]["category_sub_cb"] # ?
            estate_relevant['category_type'] = estate["seo"]["category_type_cb"] # ? sell
            # estate_relevant['locality_region_id'] = 10
...
#get_re_offers(path_to_sqlite="estate_data.sqlite", category_main="houses", category_type="sale", category_sub=[], locality_region=["Středočeský kraj"])
